...
import info.jhpc.thread.PriorityRunQueue;
import info.jhpc.thread.SharedTerminationGroup;
import info.jhpc.thread.TerminationGroup;
...
class Knapsack3 {
    ...
    public Knapsack3(int[] weights, int[] profits, int capacity, int DFSLEVELS) {
        ...
        // ****************************
        prq.run(searchFactory.make(0, capacity, 0, new BitSet(item.length), tg),
                0);
    ...
    public static class Test1 {
        public static void main(String[] args) {
            try {
                ...
                System.out.println();
                Knapsack3 ks = new Knapsack3(w, p, capacity, dfslevels);
    ...
    public static class Test3 {
        public static void main(String[] args) {
            try {
                if (args.length < 2) {
                    System.out.println("usage: java Knapsack3 length dfslevels");
                    System.exit(0);
                }
                int num = Integer.parseInt(args[0]);
                int max = 100;
                int DFSLVLS = Integer.parseInt(args[1]);
                // System.out.println("Knapsack3\t"+num+"\t"+LVLS);
                ...
                long starttime = System.currentTimeMillis();
                Knapsack3 ks = new Knapsack3(w, p, capacity, DFSLVLS);
                ks.getProfit();
                System.out.println("Knapsack3\t" + num + "\t" + DFSLVLS + "\t"
                        + (System.currentTimeMillis() - starttime));
            } catch (Exception exc) {
                System.out.println(exc);
            }
    ...
    class Search implements Runnable {
        ...
        public void run() {
            for (; ; ) {
                ...
                if (rw - item[i].weight >= 0) {
                    ...
                    b.clear(i);
                    prq.run(
                            searchFactory.make(i + 1, rw, p, (BitSet) b.clone(),
                                    tg.fork()), p + rw * item[i + 1].profitPerWeight);
        ...
        void dfs(int i, int rw, int p) {
            if (i >= item.length) {
                if (p > bestProfit) {
                    synchronized (Knapsack3.this) {
                        if (p > bestProfit) {
                            bestProfit = p;
                            Knapsack3.this.selected = (BitSet) b.clone();
